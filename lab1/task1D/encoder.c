...
int main(int argc, char **argv){
    ...
    if(mode == 1 && flag == 0)
        upperToLower(output, debug,"", flag);
    if(mode == 1 && flag ==1)
        upperToLower(output, debug,argv[index], flag);
    if(mode ==2 && flag ==0){
...
void upperToLower(FILE * output, int debug, char file[] ,int flag) 
{
    char before, after;
    int counter=0;
    char str[strlen(file)];
