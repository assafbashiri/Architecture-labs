...
virus* readVirus(FILE* reader){
    struct virus *v = calloc(sizeof(virus), sizeof(char));
    int counter = 0;
...
void kill_virus(char *fileName, int signitureOffset, int signitureSize){
    ...
    fseek(fileWrite, signitureOffset , SEEK_SET); 
    char empty = 0x90;
    for(int i=0; i<signitureSize; i++)
        fwrite(&empty, sizeof(char), 1, fileWrite);

    fclose(fileWrite);
}
link* kill_main(void *file , void *lis){
    link *start = (link*)lis;
    char signatureOffset[256], signatureSize[256];
    int signatureOffsetNum, signatureSizeNum;
    printf("Enter starting byte location: ");
    fgets(signatureOffset, sizeof(signatureOffset), stdin);
    sscanf(signatureOffset, "%d", &signatureOffsetNum);
    printf("Enter size of virus signature: ");
    fgets(signatureSize, sizeof(signatureSize), stdin);
    sscanf(signatureSize, "%d", &signatureSizeNum);

    kill_virus(file_name, signatureOffsetNum, signatureSizeNum);
    return start;
}
